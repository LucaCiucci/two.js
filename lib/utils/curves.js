...
function integrate(f, a, b, n) {
    let x = Curve.abscissas[n - 2], w = Curve.weights[n - 2], A = 0.5 * (b - a), B = A + a, i = 0, m = (n + 1) >> 1, sum = n & 1 ? w[i++] * f(B) : 0; // Handle odd n
    while (i < m) {
        const Ax = A * x[i];
...
export { Curve, getComponentOnCubicBezier, subdivide, getCurveLength, getCurveBoundingBox, integrate, getCurveFromPoints, getControlPoints, getReflection, getAnchorsFromArcData };
